...
4. Er wordt een rapport naar SonarCloud gestuurd van de kwaliteit van de code.
5. De Docker container wordt uitgerold naar een Azure Container Registry.
6. Een trigger van een Azure App Service wordt aangeroepen waardoor de nieuwe image wordt uitgerold naar de App Service die gekoppeld is aan het domein.
7. De pipeline is voltooid.

## Opdracht 5: Implementatie BDD

We maken gebruik van [Cucumber](https://cucumber.io/) voor Design Drive Development

### Feature 1

We hebben feature tests gemaakt voor een item, deze testen beschrijven de verschillende scenario's dat een item kan hebben op een pagina.

![Item feature](images/item-feature.png)
...
Een groot verschil is dat bij BDD de testen eerst in menselijke taal worden geschreven.
Dit maakt het mogelijk dat andere stakeholders ook testen kunnen schrijven.
Ook kunnen dan de testen samen met de klant worden geschreven, hierdoor kan de klant eerder stadium in het project betrokken worden.

BDD kan ook voor integratie- en acceptatietesten worden gebruikt, BDD hoeft dus niet alleen voor unit testen gebruikt te worden.
Een nadeel van BDD is dat er veel meer communicatie nodig is met de klant.
In de praktijk komt het vrijwel nooit voor dat alle feature files voor jou geschreven gaan worden.

Voor dit project was het voor ons veel handiger om gebruik te maken van TDD.
Omdat wij een team van programmeurs zijn hebben wij geen problemen met het lezen van unit tests.
Ook hoefde de klant niet bij het maken van de testen betrokken te zijn.
